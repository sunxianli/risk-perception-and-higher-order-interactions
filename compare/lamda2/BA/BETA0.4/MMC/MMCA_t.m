...
parfor ga=1:choice
    %gamma=parmters(ga);
    lamda_deta=parmters(ga);
    lamda2=medi*lamda_deta
...
plot(beta_R,rhoAG(1,:), '-o', 'Color', c.vermilion, 'MarkerSize', 4,'MarkerFaceColor', c.vermilion);
plot(beta_R, rhoAG(2,:), '-^', 'Color', c.blue,'MarkerSize', 4,      'MarkerFaceColor', c.blue);
plot(beta_R, rhoAG(3,:), '-v', 'Color', c.green,'MarkerSize', 4,     'MarkerFaceColor', c.green);

set(gcf,'DefaultTextInterpreter','latex');
xlabel('$\beta$','FontSize',15);ylabel('$\rho^{AG}$','FontSize',15);   
h=legend({'$\lambda^{*}=0.3$', '$\lambda^{*}=0.6$', '$\lambda^{*}=0.9$'}, 'Interpreter', 'latex', 'FontSize', 15);
