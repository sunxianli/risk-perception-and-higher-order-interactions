...
rho_AG=zeros(num,termi,termi);

parfor chance=1:num;
    lamda_delta_val = lamda_delta(chance);
    rho_local = zeros(termi, termi);       % 本 worker 的局部结果 (delta × lambda)
    for xun = 1:termi
       delta=xun/termi;
       lamda2=(delta*lamda_delta_val)/avg_k_delta;
        sum_AG_over_rep = zeros(1, termi);
  for rep = 1:MC_rep
    ...
    for la = 1:termi
        ...
        local_AG(la) = mean(x);   % 本次仿真在该 λ1 下的 G 密度
       
    end%lamda1

          sum_AG_over_rep = sum_AG_over_rep + local_AG;
    
  end%仿真次数 
  rho_local(xun, :) = sum_AG_over_rep / MC_rep;
    end%delta
 rho_AG(chance, :, :) = rho_local;   % 一次性写回，避免 parfor 冲突
end%lamda——delta



lamda_vals=linspace(1/termi,1,termi);
delta_vals=linspace(1/termi,1,termi);
