...
lamda_vals = linspace(1/ter, 1, ter);     % x 轴：lambda1
delta_vals = linspace(1/ter, 1, ter);     % 用于计算 lambda2
lamda_delta_vals = lamda_delta;           % z 轴层：lambda_delta（逐层悬浮）
...
for idx = 1:length(lamda_delta_vals)
    % 当前层的 lambda_delta
    lam_delta = lamda_delta_vals(idx);

    % 该层对应的 lambda2（随 delta 线性缩放）
    lamda2_vals_layer = (delta_vals * lam_delta) / avg_k_delta;

    % 与 rhoAG 的索引对应关系：Z(row=delta_idx, col=lambda1_idx)
    % 因此 meshgrid 仍然是 (lambda1 over columns, lambda2 over rows)
    [LAM1grid, LAM2grid] = meshgrid(lamda_vals, lamda2_vals_layer);

    % 颜色数据：该层的稳态密度
    Zcolor = squeeze(rhoAG(idx, :, :));   % 大小 = [length(delta_vals) x length(lamda_vals)]

    % 悬浮的 z 平面 = 当前层的 lambda_delta
    Zplane = lam_delta * ones(size(LAM1grid));

    surf(LAM1grid, LAM2grid, Zplane, Zcolor, ...
        'EdgeColor','none', 'FaceAlpha',0.95, ...
        'DisplayName', sprintf('$\\lambda_{\\delta} = %.3g$', lam_delta));
end
