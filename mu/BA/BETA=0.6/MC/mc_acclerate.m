...
plot(mu,rho_AG(1,:), '-o', 'Color', c.vermilion,'MarkerSize', 4);
 plot(mu,rho_AG(2,:), '-^', 'Color', c.blue,'MarkerSize', 4 );
 plot(mu, rho_AG(3,:), '-v', 'Color', c.green,'MarkerSize', 4);

set(gcf,'DefaultTextInterpreter','latex');
xlabel('$\mu$','FontSize',15);ylabel('$\rho^{AG}$','FontSize',15);   
h=legend({'$\gamma=0.3(MMC)$', '$\gamma=0.6(MMC)$', '$\gamma=0.9(MMC)$'}, 'Interpreter', 'latex', 'FontSize', 15);
